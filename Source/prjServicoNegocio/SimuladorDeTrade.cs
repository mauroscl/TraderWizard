...
namespace ServicoNegocio
{
	...
	public class SimuladorDeTrade
	{
			...
			while ((!blnEncontrouNegativo)) {
				...
				while ((!objRS.Eof) && (!blnEncontrouNegativo)) {
					...
					if (Convert.ToDouble(objRS.Field("Oscilacao")) < 0) {
					    ...
					    if (!double.TryParse(Convert.ToString(objRS.Field("MME21")), out var valorMedia)) {
							pdblMME21Ret = null;
						} else {
		...
		public IFRSimulacaoDiaria Simular(CotacaoDiaria pobjCotacaoDeInicioDaSimulacao)
		{
		    ...
		    var objInformacoesDoTradeDTO = new InformacoesDoTradeDTO();
			...
			decimal decValorEntradaAjustado = objInformacoesDoTradeDTO.ValorDeEntradaOriginal;
		    ...
		    if (objSetup.TemFiltro) {
				...
				double dblValorIFRMinimo = IFRMinimoAnteriorCalcular(objCotacaoDeAcionamentoDoSetup.Data, strTabelaCotacao, strTabelaIFR, strTabelaMedia
			...
			DateTime dtmDataInicial = objCalculadorData.CalcularDataProximoPeriodo(objAtivo.Codigo, objCotacaoDeAcionamentoDoSetup.Data, strTabelaCotacao);
			...
			while (!blnOperacaoEncerrada) {
                ...
                DateTime dtmDataAux = dtmDataInicial.AddMonths(1);
                ...
                DateTime dtmDataFinal = dtmDataAux.AddDays(-1);
			    ...
			    foreach (CotacaoDiaria objCotacaoDoFluxoDaSimulacao in lstCotacoes) {
					...
					if ((objCotacaoDeEntrada != null)) {
						...
						if (objCotacaoDoFluxoDaSimulacao.ValorMinimo <= objInformacoesDoTradeDTO.ValorDoStopLoss) {
							...
							objInformacoesDoTradeDTO.ValorDoStopLoss = objSetup.CalculaValorStopLossDeSaida(objCotacaoDoFluxoDaSimulacao, objInformacoesDoTradeDTO,null);
